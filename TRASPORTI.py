...
with c3:
    ...
    st.metric("Delta H2 vs BEV", f"€ {diff_h2_bev:,.0f}", f"{diff_h2_bev/total_km_life:,.2f} €/km", delta_color="inverse" if diff_h2_bev > 0 else "normal")
...
try:
    # Mappatura case-insensitive per trovare il foglio Excel corretto
    ...
    for idx, r in df_abs.iterrows():
        ...
        res.append({
            "Tecnologia": t, "Autonomia": aut_ev, "Consumo": r['Consumo'], 
    ...
    with col_g2:
        ...
        f2 = px.bar(df_final, x="Tecnologia", y="Consumo", color="Tecnologia", text_auto='.2f')
        f2.add_hline(y=diesel_val['Consumo'], line_dash="dash", line_color="black", annotation_text="Baseline Fossile")
    ...
    with col_g3:
        ...
        f3.add_hline(y=diesel_val['Eta'], line_dash="dash", line_color="black", annotation_text="Baseline Fossile")
    ...
    with col_g4:
        ...
        df_melt['Fase'] = df_melt['Fase'].replace({'E_Produzione': 'Costruzione', 'E_Manutenzione': 'Manutenzione', 'E_Carburante': 'Carburante/Uso'})
        f4 = px.bar(df_melt, x="Tecnologia", y="tCO2", color="Fase", barmode='stack', color_discrete_sequence=["#8E8E8E", "#FF7F0E", "#D62728"])
        tot_fossile = diesel_val['E_Produzione'] + diesel_val['E_Manutenzione'] + diesel_val['E_Carburante']
        f4.add_hline(y=tot_fossile, line_dash="dash", line_color="black", annotation_text="Totale Fossile")
